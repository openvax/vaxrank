...
class MutantProteinFragment(MutantProteinFragment_Fields):
    ...
    def generate_subsequences(self, subsequence_length):
        ...
        """
        if len(self.amino_acids) <= subsequence_length:
            yield (0, self)

        for subsequence_start_offset in range(len(self)):
            ...
            mutant_amino_acid_start_offset = max(
                subsequence_start_offset,
                self.mutant_amino_acid_start_offset)
            mutant_amino_acid_end_offset = min(
                subsequence_end_offset,
                self.mutant_amino_acid_end_offset)
    ...
    def top_k_subsequences(
            ...
            k,
            sort_key=lambda x: (
                -x.max_mutation_distance_from_edge,
                -x.num_mutant_amino_acids)):
        """
